...
const AppointmentForm: React.FC<AppointmentFormProps> = ({ appointment, initialDate, onSuccess, onCancel }) => {
  ...
  const handleSelectService = (service: Service) => {
    setFormData({ ...formData, servicoId: service.id!, valorAgendamento: service.valorServico });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. VERIFICAR HORÁRIO FUNCIONAMENTO
      ...
      // SALVAR AGENDAMENTO
      const payload = { ...formData, horaFinal };
      let finalApptId = appointment?.id;

      if (appointment?.id) {
        const { id, ...dataToUpdate } = payload;
        await updateDoc(doc(db, 'appointments', appointment.id), dataToUpdate);
      } else {
        const docRef = await addDoc(collection(db, 'appointments'), { ...payload, createdAt: Date.now() });
        finalApptId = docRef.id;

        // --- LÓGICA DE NOTIFICAÇÃO LOCAL ---
        const notifDoc = await getDoc(doc(db, 'configuracoes', 'notificacao'));
        if (notifDoc.exists()) {
          const notifConfig = notifDoc.data();
          if (notifConfig.ativo === true) {
            const antecedencia = notifConfig.antecedenciaMinutos || 60;
            
            // Calcular Horário Exato da Notificação para o Log
            const [h, m] = formData.horaAgendamento!.split(':').map(Number);
            const notifTimeDate = new Date();
            notifTimeDate.setHours(h, m - antecedencia, 0, 0);
            const horaNotificacao = `${String(notifTimeDate.getHours()).padStart(2, '0')}:${String(notifTimeDate.getMinutes()).padStart(2, '0')}`;

            // Agendar Notificação no Dispositivo
            const wasScheduled = await NotificationService.scheduleStudioNotification(
              finalApptId!,
            ...
            // Registrar Log na coleção 'notificacoes'
            if (wasScheduled) {
              await addDoc(collection(db, 'notificacoes'), {
                createdAt: Date.now(),
                dataNotificacao: formData.dataAgendamento,
                servicoNotificacao: currentService.displayName,
                clienteNotificacao: currentClient.displayName,
                horaNotificacao: horaNotificacao // Novo campo solicitado
              });
            }
          }
        }
      ...
      console.error(err);
      setErrorMsg("Erro ao processar agendamento.");
    } finally {
      setSaving(false);
    }
  };
  ...
  if (showSuccess) {
    return (
      <div className={`flex flex-col items-center justify-center min-h-[60vh] text-center p-6 transition-all duration-700 ease-in-out ${isExiting ? 'opacity-0 scale-95 translate-y-10' : 'opacity-100 scale-100 animate-in fade-in zoom-in duration-500'}`}>
        ...
        <p className="text-slate-500 text-sm max-w-xs mx-auto leading-relaxed">O horário foi reservado e a notificação de lembrete foi configurada.</p>
      </div>
    );
  }
    ...
    <div className="relative">
      {errorMsg && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-[340px] rounded-[3rem] p-8 shadow-2xl animate-in zoom-in duration-300 relative text-center">
            <button onClick={() => setErrorMsg(null)} className="absolute top-6 right-6 p-2 text-slate-300 hover:text-rose-500 transition-colors"><X size={20} /></button>
            <div className="flex flex-col items-center space-y-5">
              <div className="w-48 h-48 mb-2"><img src={ERROR_IMAGE} alt="Erro" className="w-full h-full object-contain" /></div>
              <div className="space-y-2">
                <h3 className="text-2xl font-serif font-bold text-slate-800">Ops, não deu!</h3>
                <p className="text-sm text-slate-500 leading-relaxed px-2">{errorMsg}</p>
              </div>
              <button onClick={() => setErrorMsg(null)} className="w-full py-4 bg-slate-800 text-white rounded-2xl text-xs font-black uppercase tracking-widest shadow-lg active:scale-95 transition-all mt-4">Entendi, vou ajustar</button>
            </div>
          </div>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-10 pb-20 max-w-xl mx-auto">
        ...
        <section className="space-y-4">
          ...
          <div className="relative">
            <div className="relative">
              ...
              <input type="text" placeholder="Pesquisar nome da cliente..." className="w-full pl-14 pr-6 py-5 bg-white rounded-3xl border border-rose-100 shadow-sm focus:outline-none focus:ring-4 focus:ring-rose-50 transition-all text-lg font-medium placeholder:text-slate-300" value={clientSearch} onChange={(e) => { setClientSearch(e.target.value); setIsClientListOpen(true); }} onFocus={() => { setIsClientListOpen(true); setIsServiceListOpen(false); }} />
            </div>
              ...
              <div className="absolute z-[160] w-full mt-2 bg-white rounded-[2rem] border border-rose-100 shadow-2xl max-h-60 overflow-y-auto custom-scrollbar animate-in fade-in slide-in-from-top-2">
                {filteredClients.length > 0 ? (
                  filteredClients.map(c => (
                    <button key={c.id} type="button" onClick={() => handleSelectClient(c)} className="w-full p-5 flex items-center justify-between hover:bg-rose-50 transition-colors text-left border-b border-rose-50 last:border-0">
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 rounded-2xl overflow-hidden bg-rose-50 flex items-center justify-center border border-rose-100">{c.fotoUrl ? <img src={c.fotoUrl} className="w-full h-full object-cover" /> : <User size={16} className="text-rose-300" />}</div>
        ...
        <section className="space-y-4">
          ...
          <div className="relative">
            <button type="button" onClick={() => { setIsServiceListOpen(!isServiceListOpen); setIsClientListOpen(false); }} className={`w-full flex items-center justify-between pl-14 pr-6 py-5 bg-white rounded-3xl border transition-all text-left shadow-sm ${isServiceListOpen ? 'border-rose-300 ring-4 ring-rose-50' : 'border-rose-100'}`}>
              <Scissors className="absolute left-5 text-rose-300" size={20} />
              <div className="flex-1">
                {selectedService ? <div className="flex justify-between items-center pr-4"><span className="text-lg font-bold text-slate-700">{selectedService.displayName}</span><span className="text-sm font-black text-rose-500">R$ {formData.valorAgendamento}</span></div> : <span className="text-lg font-medium text-slate-300">Selecione o procedimento...</span>}
              </div>
            ...
            {isServiceListOpen && (
              <div className="absolute z-[160] w-full mt-2 bg-white rounded-[2rem] border border-rose-100 shadow-2xl max-h-60 overflow-y-auto custom-scrollbar animate-in fade-in slide-in-from-top-2">
                {services.length > 0 ? (
                  services.map(s => (
                    <button key={s.id} type="button" onClick={() => handleSelectService(s)} className={`w-full p-5 flex items-center justify-between hover:bg-rose-50 transition-colors text-left border-b border-rose-50 last:border-0 ${formData.servicoId === s.id ? 'bg-rose-50/50' : ''}`}>
                      <div className="flex flex-col"><span className="text-base font-bold text-slate-700">{s.displayName}</span><span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{s.duracao} minutos</span></div>
                      <div className="flex items-center gap-4"><span className="text-sm font-black text-rose-500">R$ {s.valorServico}</span>{formData.servicoId === s.id && <CheckCircle2 size={18} className="text-rose-500" />}</div>
                    </button>
                  ))
        ...
        <section className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="relative">
              ...
              <input type="date" className="w-full pl-14 pr-6 py-5 bg-white rounded-[1.5rem] border border-rose-100 focus:outline-none focus:ring-4 focus:ring-rose-50 text-base font-bold text-slate-700 shadow-sm" value={formData.dataAgendamento} onChange={e => setFormData({...formData, dataAgendamento: e.target.value})} />
            </div>
            <div className="relative">
              <Clock className="absolute left-5 top-1/2 -translate-y-1/2 text-rose-300" size={20} />
              <input type="time" className="w-full pl-14 pr-6 py-5 bg-white rounded-[1.5rem] border border-rose-100 focus:outline-none focus:ring-4 focus:ring-rose-50 text-base font-bold text-slate-700 shadow-sm" value={formData.horaAgendamento} onChange={e => setFormData({...formData, horaAgendamento: e.target.value})} />
            </div>
          </div>
        </section>
        ...
        <section className="space-y-6">
          ...
          <div className="bg-white p-6 rounded-[2.5rem] border border-rose-100 shadow-sm space-y-6">
            <div>
              ...
              <div className="relative"><span className="absolute left-5 top-1/2 -translate-y-1/2 text-rose-500 font-bold">R$</span><input type="number" className="w-full pl-12 pr-6 py-4 bg-rose-50/30 rounded-2xl border border-rose-100 focus:outline-none focus:ring-4 focus:ring-rose-50 text-2xl font-black text-rose-500" value={formData.valorAgendamento} onChange={e => setFormData({...formData, valorAgendamento: Number(e.target.value)})} /></div>
            </div>
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {statusOptions.map((opt) => (
                  <button key={opt.label} type="button" onClick={() => setFormData({...formData, status: opt.label})} className={`px-4 py-3 rounded-2xl flex items-center gap-2 text-[10px] font-black uppercase tracking-tighter transition-all border-2 ${formData.status === opt.label ? 'bg-slate-800 border-slate-800 text-white shadow-lg scale-105' : `${opt.color.split(' ')[0]} border-transparent text-slate-500 hover:border-rose-100`}`}><opt.icon size={14} />{opt.label}</button>
                ))}
              </div>
            </div>
            <div>
              <label className="text-[10px] font-black text-slate-400 uppercase ml-2 tracking-widest mb-2 block">Notas Adicionais</label>
              <div className="relative"><Info className="absolute left-5 top-5 text-rose-300" size={20} /><textarea rows={3} className="w-full pl-14 pr-6 py-4 bg-slate-50 rounded-2xl border border-slate-100 focus:outline-none focus:ring-4 focus:ring-rose-50 text-sm font-medium resize-none placeholder:text-slate-300" placeholder="Observações importantes..." value={formData.observacao} onChange={e => setFormData({...formData, observacao: e.target.value})} /></div>
            </div>
          </div>
        </section>

        <div className="pt-4">
          <button type="submit" disabled={saving} className="w-full py-6 bg-gradient-to-tr from-rose-600 to-pink-500 text-white rounded-[2.5rem] text-lg font-black uppercase tracking-[0.2em] shadow-2xl shadow-rose-200 active:scale-95 transition-all flex items-center justify-center gap-3 disabled:opacity-50">{saving ? <Loader2 size={24} className="animate-spin" /> : (appointment ? <><CheckCircle2 size={24} /> Atualizar Agenda</> : <><CheckCircle2 size={24} /> Confirmar Horário</>)}</button>
        </div>
      </form>
    </div>
  );
